...
// this version is to count the total comparsions
void ComputeSetIntersection::ComputeCandidates(const VertexID* larray, const ui l_count,
                                               ...
                                               VertexID* cn, ui &cn_count, unsigned long long &count) {
	if (l_count / 50 > r_count || r_count / 50 > l_count) {
        return ComputeCNGallopingAVX2(larray, l_count, rarray, r_count, cn, cn_count, count );
    }
    else {
        return ComputeCNMergeBasedAVX2(larray, l_count, rarray, r_count, cn, cn_count, count);
    }
...
// this version is to count the comparsions
void ComputeSetIntersection::ComputeCNGallopingAVX2(const VertexID* larray, const ui l_count,
                                                    const VertexID* rarray, const ui r_count,
                                                    VertexID* cn, ui &cn_count, unsigned long long& count) {
    cn_count = 0;
...
//  this version is to count the total number of comparsions
void ComputeSetIntersection::ComputeCNMergeBasedAVX2(const VertexID* larray, const ui l_count,
                                                     const VertexID* rarray, const ui r_count,
                                                     VertexID* cn, ui &cn_count, unsigned long long &count) {
    cn_count = 0;
