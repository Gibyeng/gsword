...
void Graph::loadGraphFromFile(const std::string &file_path) {
    ...
    int process = 0;
    // check while there are no degree vertices
//    printf("allocate visited  \n");
//    bool* visited = new bool [vertices_count_];
//    memset(visited, 0, vertices_count_ * sizeof(bool));
    while (infile >> type) {
...
//            visited[id] = 1;
            labels_[id] = label;
            // adjust unvisited vertice offsets
//            if(visited[id - 1] == 0){
...
//            }
        ...
        process++;
        if(process % 1000000000 == 0){
        	printf("now prcess line %d\n",process);
        }
    }
...
//    		printf("process visited vertice %d \n", i);
    		std::sort(neighbors_ + offsets_[i], neighbors_ + offsets_[i + 1]);
//    	}
    }
    printf("Build reverse index...\n");
//    delete[] visited;
    BuildReverseIndex();
